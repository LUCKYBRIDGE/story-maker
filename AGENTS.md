...
5. 작업에 관련된 품질 관점을 선택한다. 역할명 나열이나 별도 검토 문서는 필수가 아니다.
6. 실행 주체와 검증 주체는 `docs/operations/github-first-hybrid-development.md`의 등급을 따른다.

## 2. 문서와 소스의 우선순위

다음은 자료의 역할과 충돌 해석을 위한 목록이며 전체 필독 목록이 아니다. 현재 상태와 해당 작업 카드부터 필요한 절만 읽는다. 구현 사실은 소스/실행으로, 의도와 제약은 승인된 제품 계약으로 판단한다. 코드와 요구가 다르다는 이유만으로 요구를 폐기하지 않는다.
...
제품 의도는 상세 설계, 작업별 의도·인수 조건은 해당 카드, 현재 진행 상태는
`docs/storygame-development-status.md`에서만 관리한다. 실행 계획은 순서나 단계 계약이
바뀔 때만 갱신한다. 같은 상태를 여러 문서에 복사하거나 별도 intent.md를 추가하지 않는다.
...
아래 표에서 현재 변경과 관련된 질문만 적용한다. 별도 페르소나 계획이나 전체 표 검토를 요구하지 않는다.
...
6. **검증**: 해당 작업의 최소 검사와 영향을 받는 실제 흐름을 확인한다. 전체 릴리스 문턱은 릴리스 대상일 때 적용한다.
...
2026-09-09 사용자 결정으로 선택지 2/3개와 컷 연결·분기·합류는 BR-01 구현 범위다.
기존 선형 작품 호환은 계속 보존한다.
