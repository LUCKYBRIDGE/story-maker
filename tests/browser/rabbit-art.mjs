...
const output=process.env.QA_OUTPUT || '/tmp/rabbit-art/browser';
...
try {
 for(const [width,height] of [[1365,900],[820,1180],[390,844]]) {
  ...
  const route=url+'classic/rabbit';
  ...
  for(let i=1;i<=56;i++) {
   await page.locator('.reader-story-info small').filter({hasText:new RegExp(`^${i} / 56$`)}).waitFor({state:'attached'});
   ...
   for (const actor of layout.actors) {
    assert.ok(actor.rect.x >= -1 && actor.rect.right <= width + 1, `${width} cut ${i}: actor width`);
    assert.ok(actor.rect.y >= -1, `${width} cut ${i}: actor top`);
    assert.ok(actor.rect.bottom <= layout.dialogue.top + 1, `${width} cut ${i}: actor above dialogue`);
   }
   if (i === 53 || i === 54 || i === 55) assert.equal(layout.actors.length, 2, 'layered escape and final conversation');
   if (i === 56) assert.equal(layout.actors.length, 1, 'turtle alone');
   if ([26,27,28,29,30,31,49,50,51,52].includes(i)) {
    assert.equal(layout.actors.length, 1, 'one shared riding actor');
    assert.ok(Math.abs(layout.actors[0].rect.x + layout.actors[0].rect.width / 2 - width / 2) < 2, 'riding pair centered');
    assert.equal(await page.locator('.story-stage-actor.listener').count(), 0, 'shared actor stays bright for either voice');
   }
   await page.screenshot({path:`${output}/${width}-${String(i).padStart(2,'0')}.png`});
   results.push({width,cut:i,...layout});
   if(i<56) await page.getByRole('button',{name:'다음 컷',exact:true}).click();
  }
  assert.ok(await page.getByRole('button',{name:'다음 컷',exact:true}).isDisabled());
  await page.getByRole('button',{name:'이전',exact:true}).click();
  assert.equal(await page.locator('.reader-story-info small').textContent(),'55 / 56');
  await page.getByRole('button',{name:'다음 컷',exact:true}).click();
  await page.close();
 }
 await writeFile(`${output}/layout.json`,JSON.stringify(results,null,2));
 console.log('PASS: 56 cuts × 3 viewports, loaded images, no page overflow, previous/next, final cut');
