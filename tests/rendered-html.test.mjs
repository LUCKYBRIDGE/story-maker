...
test("화자·이미지·외부 자료가 분리된 편집 도구로 유지된다", async () => {
  ...
  assert.equal(recommendedAssets.length, 121);
  assert.equal(
  ...
  assert.equal(
    recommendedAssets.filter(
      (asset) => asset.story === "선녀와 나무꾼" && asset.type === "character",
    ).length,
    32,
  );
  assert.equal(
  ...
  assert.equal(
    recommendedAssets.filter(
      (asset) => asset.story === "흥부와 놀부" && asset.type === "character",
    ).length,
    10,
  );
  assert.equal(
    recommendedAssets.filter(
      (asset) => asset.story === "흥부와 놀부" && asset.type === "background",
    ).length,
    14,
  );
  assert.ok(
