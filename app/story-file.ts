...
export const KNOLSTORY_MIME = "application/vnd.knolstory+json";
export const NOLSTORY_MIME = "application/vnd.nolstory+json";
export const MAX_KNOLSTORY_BYTES = 10 * 1024 * 1024;
export const MAX_NOLSTORY_BYTES = MAX_KNOLSTORY_BYTES;
type Manifest = { format: "knolstory" | "nolstory"; version: 1; kind: "project" | "shared"; exportedAt: string; appVersion: string };
type AssetReference = { kind: "builtin"; id: string };
export type KnolstoryProjectFile = { manifest: Manifest & { kind: "project" }; assets: AssetReference[]; draft: StoryDocumentEnvelope; playback: StoryDocumentEnvelope | null };
export type KnolstorySharedFile = { manifest: Manifest & { kind: "shared" }; assets: AssetReference[]; story: StoryDocumentEnvelope; sharing: { allowRemix: boolean; authorDisplayName: string } };
export type KnolstoryFile = KnolstoryProjectFile | KnolstorySharedFile;
export type NolstoryProjectFile = KnolstoryProjectFile;
export type NolstorySharedFile = KnolstorySharedFile;
export type NolstoryFile = KnolstoryFile;
export type StoryFileResult = { ok: true; file: KnolstoryFile } | { ok: false; message: string };
...
export function parseKnolstoryFile(text: string): StoryFileResult {
  ...
  try {
    ...
    if (manifest.kind === "project") {
      ...
      file = { manifest: manifest as KnolstoryProjectFile["manifest"], draft, playback, assets: [] };
    } else {
      ...
      file = { manifest: manifest as KnolstorySharedFile["manifest"], story, sharing: { allowRemix: value.sharing.allowRemix, authorDisplayName: value.sharing.authorDisplayName }, assets: [] };
    }
    const expected = references(file.manifest.kind === "project" && "draft" in file
      ? [file.draft.project, ...(file.playback ? [file.playback.project] : [])] : [(file as KnolstorySharedFile).story.project]);
    ...
    return failure(error instanceof SyntaxError ? "파일 내용을 읽지 못했어요. 정상적인 .knolstory 파일인지 확인해 주세요." : error instanceof Error ? error.message : "파일을 열지 못했어요.");
  }
}

export function createKnolstoryProject(entry: ProjectEntry, now = new Date().toISOString()): KnolstoryProjectFile {
  return { manifest: { format: "knolstory", version: 1, kind: "project", exportedAt: now, appVersion: STORY_PROJECT_APP_VERSION },
...
export function createKnolstoryShared(project: StoryProject, allowRemix = false, now = new Date().toISOString()): KnolstorySharedFile {
  ...
  return { manifest: { format: "knolstory", version: 1, kind: "shared", exportedAt: now, appVersion: STORY_PROJECT_APP_VERSION },
...
export function encodeKnolstoryFile(file: KnolstoryFile): string {
  const text = JSON.stringify(file);
  const parsed = parseKnolstoryFile(text);
  if (!parsed.ok) throw new Error(parsed.message);
  return text;
}
export async function readKnolstoryFile(file: File): Promise<StoryFileResult> {
...
export function downloadKnolstoryFile(file: KnolstoryFile, title: string) {
  const blob = new Blob([encodeKnolstoryFile(file)], { type: KNOLSTORY_MIME });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `${(title || "제목 없는 이야기").replace(/[\\/:*?"<>|\u0000-\u001f]/g,"_").slice(0,80)}-${file.manifest.kind}.knolstory`;
  link.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}

export const parseNolstoryFile = parseKnolstoryFile;
export const createNolstoryProject = createKnolstoryProject;
export const createNolstoryShared = createKnolstoryShared;
export const encodeNolstoryFile = encodeKnolstoryFile;
export const readNolstoryFile = readKnolstoryFile;
export const downloadNolstoryFile = downloadKnolstoryFile;
