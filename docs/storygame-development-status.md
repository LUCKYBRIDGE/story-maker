...
- 2026-09-16 사용자 직접 요청, **DONE — 로컬 구현·검증 완료**, Work Lead · A/B. 첨부 대본과 기존 두 원작읽기를 작업 계약으로 적용.
- 기준 `origin/main` `6846bc0`, branch `codex/seonnyeo-classic-reading`, 시작 시 작업 트리 깨끗함.
- 범위: 6장·83컷 원작 데이터·표지·기획, 원작 전용 경로와 서재 진입, 기존 자산 연출과 하늘나라 배경, 관련 회귀 검사. 분기형 놀스토리·학생 저장본·플레이어 구조는 유지.
- 인수: 첨부 사건·문장·순서·화자 보존(회상 화자는 사슴으로 정규화), 선형 완주, 장별 메타데이터, 3개 화면 크기에서 진입·이전/다음·책갈피 복원·서재 복귀.
- 연출: [컷 배치 원칙·추가 배경·내장 imagegen 프롬프트](design/seonnyeo-classic-art.md). 기존 선녀·나무꾼과 환경 재사용, 하늘나라 배경 1종(약 297KB) 추가. 전용 그림이 없는 인물·사물은 배경과 대본으로 전달.
- 사실 보정: 기획 질문의 두 번째 금기 전달자는 선녀이므로 “사슴과 선녀”로 수정. 대본 문장·순서는 유지.
- 재진입 보완: 이번 원작 경로에서 서재로 돌아갈 때 해당 작품의 임시 화면 상태만 해제. 표지에서 영속 책갈피를 제시하여 첫 컷 자동저장으로 덮이지 않도록 함.
- 생성기: 재생성 시 기존 별주부전 명칭·검색 별칭을 유지하도록 보정. 기존 187개 자산 데이터는 그대로이며 새 배경만 추가.
- A: `npm run check` 통과, 원작·책갈피·기본 작품 회귀 7/7 통과. `npm run build:github -- --webpack` 통과(기본 Turbopack은 기존 로컬 영속 캐시 파싱 오류, 캐시 삭제 없이 지원되는 webpack 경로 사용).
- B: `classic-reading` 통과. `seonnyeo-classic`에서 1365×900/820×1180/390×844 각각 83컷 전수 이동, 이미지 로드·글 잘림·가로 넘침·끝 컷·이전/다음·키보드 진입·서재 복귀·같은 탭 재진입/새 창 책갈피 복원 통과. 주요 장면 화면 직접 확인. 증거 `/tmp/seonnyeo-classic-qa-final/results.json` 및 같은 폴더의 스크린샷. 실제 모바일 기기 검수는 미포함.
- 커밋·푸시·공개 배포는 이번 요청 범위에 포함하지 않음. 다음 READY 없음.

## 최근 완료: RT-ART-01 토끼전 원작 읽기 이미지 보강

- 2026-09-15 사용자 직접 요청, **DONE — 구현·로컬 검증 완료, PR #47 검토 대기**, Work Lead · G/A/B. 사용자 요청의 56컷 인수 조건을 작업 계약으로 적용.
...
- G: 구현 커밋 `0f0afa0` 및 문서 후속 커밋 push, [PR #47](https://github.com/LUCKYBRIDGE/story-maker/pull/47) 생성. 원격 자동 검증은 [해당 HEAD Checks](https://github.com/LUCKYBRIDGE/story-maker/pull/47/checks)에서 확인.
- 다음 READY 없음.
...
- 소스 저장소: GitHub `LUCKYBRIDGE/pinky-ne-site` (`main` `1a3054f`).
- 서사 및 규칙 완성:
  1. 날개옷 판타지 규칙 일원화: 흙 속에 오래 묻혀 약해졌으나 땅 밖으로 꺼내는 순간 추가적인 훼손은 멈추며, 품에 안으면 불안하게 흔들리던 은빛이 안정됨. 약해진 날개옷으로 하늘길을 여는 기한은 “이 계절의 마지막 보름밤”에 한하며, 하늘 귀환 후 은하수의 기운을 받으면 본래의 힘을 온전히 회복하여 훗날 재방문이 가능함(Ending 3 재회 논리 확보).
  2. 사슴 및 공간 설정: 사슴의 은혜 갚기나 의도적 중매·절도 설정은 일절 배제하여 윤리적 책임을 외부에 전가하지 않음. 첫 폭풍에 날아가 쓰러진 소나무 뿌리 아래 흙에 묻혔다가 두 번째 폭풍으로 소나무가 뽑히며 드러나는 자연 서사를 확립하여 현재 이미지 자산(`BG-forest-roots`, `BG-roots-robe`)과 100% 일치.
  3. 선택 4-B 선녀의 주체성: 나무꾼이 하루 숨겨둔 동안(H2) 선녀가 직접 계곡과 동쪽 숲을 돌아보며 날개옷 흔적을 수색함. 나무꾼에게 거짓 수색 연기를 강요하지 않으면서 숨겨둔 옷에 대한 부끄러움과 죄책감을 심화.
  4. 네 엔딩의 위계 없는 정체성 확립:
     - Ending 1 「존중과 이별」: 자유를 온전히 돌려주었고, 선녀는 고향인 하늘로 돌아가기로 스스로 선택함. 나무꾼은 그 선택을 전적으로 존중하며 아름다운 작별을 나눔.
     - Ending 2 「지상에서의 약속」: 자유를 온전히 돌려주었고, 선녀는 지상의 삶 자체(계절, 스스로 일구는 일상)를 사랑하여 지상에 머물기로 스스로 선택함. 보상적 결혼이 아닌 동등한 동반자로서 새로운 삶의 약속 시작.
     - Ending 3 「늦은 고백」: HA(이튿날 자발적 고백: 하루 숨김의 섭섭함+먼저 말해준 고마움) vs HB(질문 후 고백: 묻지 않았다면 말하지 않았을까 하는 불안과 슬픔) 감정 차이 심화. 재회 시 은하수 옷 회복 및 다시 이야기해볼 수 있는 겸허한 가능성 보존.
     - Ending 4 「돌이킬 수 없는 침묵」: 직접 질문에도 거짓말하고 은폐한 반복된 선택으로 인한 신뢰 붕괴. 초자연적 처벌 대신 빈 장롱을 마주하는 영원한 침묵으로 결말.
  5. 선택지 문구: 도덕 시험처럼 보이지 않도록 당시 나무꾼의 심리와 자기합리화가 드러나는 문장으로 정련.
- 생성/검증 안전성:
  - `pinky-ne-site` 원본 커밋(`1a3054f`) 완료 후 공식 생성기를 통해 `app/story-examples.generated.json` 재생성 (1689컷, sourceCommit 일치).
- 증거:
  - `pinky-ne-site`: `npm run test:seonnyeo` (7/7 전수 통과, 70장면, 725비트, 64경로, 162챕터, 1689컷).
  - `story-maker`: `npm run check` (0 errors), `npm test` (207/207 통과), `QA_SUITES=seonnyeo-e2e node tests/browser/run-smoke.mjs` (3/3 통과, 1689컷 import 검증).
