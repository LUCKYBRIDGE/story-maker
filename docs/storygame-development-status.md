...
| 로컬 작업 트리 | `e3415df` 기준 U2-01/02 로컬 미커밋 변경. 사용자 기존 변경 없이 시작, push/배포 안 함 |
| `npm run check` | 2026-09-08 U2-02 최종 재검사 통과 |
| `npm test` | 2026-09-08 Vinext 빌드 및 122/122 통과 |
...
| 최신 전체 검사 기록 | 2026-09-08 `npm run check`, Vinext 빌드, `npm test` 122/122 통과 |
| GitHub 기준선 | PR #1 브랜치 `u1-10-chat-player-readability-20260905` 및 CI 통과; 이후 HEAD는 세션 시작 시 재조회 |
| 이번 좁은 검사 | U2-01/02: 두 이어쓰기·입력/적용/플레이/복귀·분할/되돌리기·HTTP fallback ID·이중 rAF 복귀·모바일 가이드 정리. QA 기록 참조 |
| 새 UI | U1 완료 이력 유지. U2-01/02 로컬 구현 완료. 물리 기기 IME/실터치·160자 적정성은 별도 미검증 |
