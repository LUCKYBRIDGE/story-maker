...
## 현재 작업: SHELF-01 서재 '새 이야기' 양장본 디자인 및 선반 배치 조화 개선

- 2026-09-13 사용자 피드백(서재 '새 이야기' 디자인 미관 불일치, 텍스트 분리 및 상단 선반 뚫림 결함)으로 진행, **DONE — 2026-09-13 로컬 구현·검증**, Work Lead · G/A/B.
- 기준 `origin/main` `67a05f6`, branch `codex/library-blank-book-aesthetic`.
- 기존 반투명 철사형 와이어프레임 `.blank-book-cover`를 기존 스토리북("옹고집전", "토끼와 자라")과 완전히 동일한 규격·품격을 지닌 프리미엄 양장본(`.student-book.blank-book-cover.blank-student-book`)으로 전환.
- 텍스트 줄바꿈 방지(`white-space: nowrap; word-break: keep-all;`) 및 `.shelf-book` 폭 비율 정돈(`.52`)으로 데스크톱/태블릿(1280×800 등)에서 선반 칸 높이를 초과해 상단 목재 선반을 뚫고 솟아오르는 문제 원천 차단.
- 상단 에디션 `나만의 이야기`, 중앙 양각 금빛 원형 씰 `＋`, 안정적인 타이틀 `새 이야기`, 부제 `직접 쓰기`로 중복 문구 정돈 및 학생 창작 의욕 고취.
