...
## 현재 작업: SN-NARRATIVE-02 선녀와 나무꾼 후반부 서사(HA-Ending 3 감정선 일치·선녀옷 기한 인과·Ending 4 대사 정제) 개선 및 1692컷 동기화

- 2026-09-14 사용자 직접 요청(HA-Ending 3 감정 불일치 해결 및 confessionTiming 조건부 대사 분기, Ending 3 관계 요약 확장, 선녀옷의 '빛 감소 중단'과 '이번 계절 마지막 복귀 기회' 인과 연결, Ending 4 옷 빛 중단 묘사 보강 및 설명적 대사 정제, 원본 커밋·1692컷 전수 동기화 및 검증), **DONE — 2026-09-14 로컬 구현·검증**, Work Lead · G/A/B.
- 기준 origin/main `7983f5a`, branch `codex/seonnyeo-e3-ha-narrative-sync`.
- 소스 저장소: GitHub `LUCKYBRIDGE/pinky-ne-site` (branch `codex/seonnyeo-e3-ha-narrative-sync`, commit `ea1f338`).
- 서사 개선:
  1. HA-Ending 3 감정 불일치 해결: `confessionTiming` 상태(`nextDay` vs `afterQuestion`)를 도입하여, 선택 4H에서 이튿날 아침 선녀가 묻기 전에 자발적으로 털어놓은 HA 경로의 경우 Ending 3 후반(장면 E3-5)에서 하루 숨김에 대한 섭섭함과 스스로 먼저 말해준 행동에 대한 고마움을 회상하도록 대사 분리(“하루 동안 옷을 숨긴 건 아직도 섭섭해요”, “내가 묻기 전에 당신이 직접 말한 것도 계속 생각났어요”). 반면 직접 추궁 후 고백한 HB 경로는 기존의 강한 감정(“한동안 당신이 정말 미웠어요”, “마지막에는 결국 말해준 것도 생각났어요”)을 유지. Ending 3 핵심 대사(“용서하러 온 건 아니에요”, “다시 이야기해볼 수 있는지 보려고 왔어요”, “그거면 충분하오”, “충분한지는 앞으로 봐야죠”)는 온전히 보존.
  2. Ending 3 관계 요약 최신화: 자발적 고백과 추궁 후 고백 두 경로 모두를 포괄하도록 “진실을 바로 말하지는 못했지만 끝내 거짓말로 덮지는 않고 선녀에게 옷을 돌려준다. 고백 시점에 따라 상처의 정도는 달라지지만 관계는 즉시 회복되지 않으며, 시간이 지난 뒤 다시 대화할 가능성이 남는다”로 정의 갱신.
  3. 선녀옷 '빛 감소'와 '마지막 복귀 기회' 인과 명확화: 흙 속에 오래 방치되어 힘이 약해졌으나 선녀의 손에 돌아오면 은빛 추가 소멸이 멎는다는 규칙과, 이미 힘을 너무 많이 잃었기에 남은 힘으로 하늘길을 다시 열 수 있는 기회는 이번 계절이 마지막이라는 설명을 R1, HA1, E3-1에 일관되게 정립.
  4. Ending 4 빛 규칙 보완: 장롱에서 선녀가 옷을 찾아 품에 안았을 때 위태롭게 흔들리던 은빛이 더 이상 흐려지지 않고 멎었다는 해설(E4-3)을 추가하여 모든 반환 경로의 물리 법칙 통일.
  5. Ending 4 대사 정제: 주제를 직접 설명하던 상담조 문장(“신뢰는 완전히 무너졌어요”, “내 삶의 모든 선택”)을 걷어내고, 선녀의 감정과 질문(“돌아갈지 남을지는 내가 결정할 일이었어요”, “그런데 당신은 내가 선택할 기회조차 주지 않았어요”, “내가 직접 물었을 때도 당신은 내 눈을 보며 아무것도 없다고 거짓말했고요”, “이제 내가 당신 말을 어떻게 믿어요?”)으로 신뢰 붕괴의 아픔을 직접적으로 전달.
  6. Ending 1~4 정체성 및 초중반 관계 형성 흐름 100% 보존.
- 생성/검증 안전성:
  - `pinky-ne-site` 원본 커밋(`ea1f338`) 완료 후 공식 생성기를 통해 `app/story-examples.generated.json` 재생성 (1692컷, sourceCommit 일치).
- 증거:
  - `pinky-ne-site`: `npm run test:seonnyeo` (7/7 전수 통과, 70장면, 725비트, 64경로, 162챕터, 1692컷).
  - `story-maker`: `npm run check` (0 errors), `npm test` (207/207 통과), `QA_SUITES=seonnyeo-e2e node tests/browser/run-smoke.mjs` (3/3 통과, 1692컷 import 검증), `npm run qa:smoke` (5/5 전체 스위트 1365/390 반응형 통과).
